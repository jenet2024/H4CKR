...
:root {
  ...
  --card-solid:  ${isDark ? "#050e05"                    : "#ffffff"};
...
@keyframes robotHover{ 0%,100%{transform:translateY(0)} 50%{transform:translateY(-10px)} }
...
@keyframes antPulse  { 0%,100%{opacity:.8} 50%{opacity:1} }
...
/* ── Theme toggle ── */
.theme-toggle { width:44px; height:24px; border-radius:12px; cursor:pointer; border:1px solid var(--green-dim); background:var(--green-faint); position:relative; transition:all .3s; flex-shrink:0; }
.theme-toggle-knob { position:absolute; top:2px; width:18px; height:18px; border-radius:50%; background:var(--green); transition:all .3s; display:flex; align-items:center; justify-content:center; font-size:10px; left:${isDark?"2px":"22px"}; }
...
// ═══════════════════════════════════════════════════════════════════
// CYBER BACKGROUND (adapts au thème)
// ═══════════════════════════════════════════════════════════════════
function CyberBackground({ theme }: { theme: Theme }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.strokeStyle = `rgba(${col},${isDark ? "0.022" : "0.04"})`;
      ctx.lineWidth = 1;
      ...
      pts.forEach((a, i) => pts.slice(i + 1).forEach(b => {
        ...
        if (d < 100) {
          ...
          ctx.strokeStyle = `rgba(${col},${(.12 * (1 - d / 100)).toFixed(3)})`; ctx.lineWidth = .5; ctx.stroke();
        }
      }));
...
// ═══════════════════════════════════════════════════════════════════
const ROBOT_CFG = [
  { p: "#00ff41", s: "#00cc35", a: "#00e5ff", label: "NEXUS-01", unit: "U-001" },
  { p: "#00e5ff", s: "#0099bb", a: "#00ff41", label: "NEXUS-02", unit: "U-002" },
  { p: "#ff9900", s: "#cc7700", a: "#ffdd00", label: "NEXUS-03", unit: "U-003" },
  { p: "#cc44ff", s: "#8822cc", a: "#ff44cc", label: "NEXUS-04", unit: "U-004" },
  { p: "#ff003c", s: "#cc0030", a: "#ffd700", label: "PHANTOM",  unit: "U-012" },
];

// Light-mode versions (plus saturées, lisibles sur fond clair)
const ROBOT_CFG_LIGHT = [
  { p: "#009922", s: "#006614", a: "#0077cc", label: "NEXUS-01", unit: "U-001" },
  { p: "#0077cc", s: "#005599", a: "#009922", label: "NEXUS-02", unit: "U-002" },
  { p: "#cc6600", s: "#994400", a: "#aa8800", label: "NEXUS-03", unit: "U-003" },
  { p: "#8822cc", s: "#551188", a: "#cc2288", label: "NEXUS-04", unit: "U-004" },
  { p: "#cc0022", s: "#880011", a: "#aa7700", label: "PHANTOM",  unit: "U-012" },
];

function CyberpunkRobot({
  idx = 0,
  animate = true,
  theme,
}: {
  ...
  return (
    <div
      style={{
        width: 380,          // <<< ROBOT TRÈS GRAND
        height: 520,         // <<< PROPORTIONNEL
        position: "relative",
        display: "flex",
        alignItems: "center",
  ...
  return (
    <button
      ...
      style={{
        ...
        fontFamily: "var(--font-hud)", fontSize: 9, letterSpacing: 1.5,
        color: "var(--green)", flexShrink: 0,
      }}
    >
      <span style={{ fontSize: 14, lineHeight: 1, animation: "themeSwitch .4s ease", display: "inline-block" }}>
        {isDark ? "🌙" : "☀️"}
      </span>
...
// ROBOT GUIDE
// ═══════════════════════════════════════════════════════════════════
function RobotGuide({ text, onDone, accent = "#00ff41", theme }: { text: string; onDone?: () => void; accent?: string; theme: Theme }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => { if (idx.current < text.length) { setDisplayed(text.slice(0, ++idx.current)); } else { clearInterval(id); setDone(true); } }, 18);
  ...
  return (
    <div style={{ display: "flex", gap: 20, alignItems: "flex-start", padding: "20px 28px", background: isDark ? "rgba(0,20,0,0.6)" : "rgba(230,248,230,0.85)", borderBottom: "1px solid var(--border-hud)", animation: "fadeIn .4s ease" }}>
      ...
      <div style={{ flex: 1 }}>
        ...
        {done && onDone && (<button className="hud-btn" onClick={onDone} style={{ marginTop: 10, fontSize: 10 }}>Commencer la mission →</button>)}
      </div>
    </div>
  );
}
...
function SteganoImage({ src, hiddenText }: { src: string; hiddenText: string }) {
  return (
    <div
    ...
    >
      <img
        ...
        style={{
          width: "100%",
          borderRadius: "8px",
          filter: "brightness(0.45) drop-shadow(0 0 12px rgba(120,0,30,0.5))",
          transition: "0.3s ease",
        }}
      />

      <div
        className="stegano-overlay"
        style={{
          position: "absolute",
          inset: 0,
          display: "flex",
...
function EnigmaCard({ enigma, index, isLocked, onSolve, theme }: {
  ...
  if (isLocked) {
    return (
      <div style={{
        background: isDark ? "rgba(0,0,0,0.35)" : "rgba(200,220,200,0.4)",
        ...
        <div>
          <div style={{ color: "var(--green-dim)", fontFamily: "var(--font-hud)", fontSize: 12 }}>
            Énigme {index + 1} — {enigma.title}
          </div>
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.correct) {
        setSolved(true);
        onSolve(res.badge_earned ?? undefined);
      } else {
        setShake(true);
      ...
      setMsg({ text: err?.detail ?? "Erreur", ok: false });
    } finally {
      setLoading(false);
    }
  };
  ...
  const askHint = async () => {
    try {
      const r = await gameApi.requestHint(enigma.id);
      setHint(r.hint);
    } catch {}
  };
  ...
  return (
    <div
      className={`game-card ${solved ? "solved" : ""}`}
      style={{
        padding: 24,
        marginBottom: 14,
        animation: `fadeUp .35s ease ${index * .08}s both`,
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{
            ...
            {enigma.points}
            <span style={{ fontSize: 9, opacity: 0.5, marginLeft: 2 }}>PTS</span>
          </span>
        </div>
      </div>
      ...
      {/* DESCRIPTION */}
      <div style={{
        background: isDark ? "rgba(0,0,0,0.3)" : "rgba(220,240,220,0.5)",
      ...
      {/* IMAGE STEGANO AVEC SCAN + OVERLAY */}
      {enigma.type === "stegano" && (
        <div
          className="image-container"
          style={{
        ...
        >
          <img
             src={`${API}/assets/${enigma.file_path}?v=${performance.now()}`}
            alt="Fichier suspect"
            style={{
              maxWidth: "100%",
              maxHeight: 280,
    ...
    />
      ...
      {enigma.file_path?.match(/\.(wav|mp3|ogg|flac)$/i) && (
        <div style={{ marginBottom: 16 }}>
          <div style={{
            ...
            marginBottom: 8, fontFamily: "var(--font-hud)"
          }}>
            🎧 SIGNAL AUDIO — Analysez attentivement
          </div>
          <div style={{
            background: isDark ? "rgba(0,0,0,0.5)" : "rgba(220,240,220,0.6)",
            border: `1px solid ${color}30`, borderRadius: 8, padding: "16px 20px"
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 10 }}>
              <div style={{ fontSize: 28, animation: "pulse 2s ease-in-out infinite", flexShrink: 0 }}>📻</div>
              <div style={{ flex: 1 }}>
                <div style={{
                  color, fontFamily: "var(--font-hud)", fontSize: 10,
                  letterSpacing: 2, marginBottom: 8
                }}>
                  {enigma.file_path.split("/").pop()?.toUpperCase()}
                </div>
                <audio
                  controls
                  preload="metadata"
            ...
            <div style={{
              color: isDark ? "rgba(255,165,0,0.65)" : "rgba(150,80,0,0.8)",
              fontSize: 11, fontFamily: "var(--font-mono)", lineHeight: 1.5
            }}>
              💡 Importez dans Audacity → Effect → Reverse pour révéler le message caché.
            </div>
          </div>
        </div>
      )}
        ...
        <form onSubmit={submit}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              placeholder={cfgBase.placeholder}
              autoComplete="off"
            />
      ...
      {msg && (
        <div style={{
          ...
          borderRadius: 4,
          fontSize: 13,
          background: msg.ok ? "var(--green-faint)" :
            isDark ? "rgba(255,0,60,0.07)" : "rgba(200,0,30,0.07)",
          border: `1px solid ${
            msg.ok ? "var(--green-dim)" :
            isDark ? "rgba(255,0,60,.3)" : "rgba(180,0,20,.3)"
          }`,
      ...
      {hint && (
        <div style={{
          ...
          borderRadius: 4,
          fontSize: 12,
          background: isDark ? "rgba(255,200,0,0.04)" : "rgba(180,140,0,0.07)",
          border: isDark ? "1px solid rgba(255,200,0,0.2)" :
            "1px solid rgba(140,100,0,0.25)",
          color: isDark ? "#ffc800" : "#7a5c00",
          fontFamily: "var(--font-mono)",
...
function GuidePage({ theme }: { theme: Theme }) {
  ...
  const sections = [
    ...
    { icon: "🖼️", title: "Stéganographie & Métadonnées", content: "Les indices sont cachés dans les métadonnées EXIF des images.\nDans le terminal : extract <fichier.png>\nCherchez le champ 'Comment'." },
    { icon: "🎧", title: "Analyse Audio", content: "Les messages audio sont prononcés à l'envers.\nOutil : Audacity (gratuit)\n1. Importez le fichier WAV\n2. Effect → Reverse\n3. Écoutez le message révélé" },
    ...
    { icon: "🏅", title: "Badges & Classement", content: "🩸 First Blood · 🔓 Déchiffreur · 🖼️ Stega Master · 🎧 Détective Audio · 🦅 Sans Filet · 🎩 Black Hat · ⚡ Speed Hacker" },
...
function ContactPage({ theme }: { theme: Theme }) {
  ...
  return (
    <div style={{ animation: "fadeUp .3s ease", maxWidth: 660, margin: "0 auto" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28 }}><span style={{ fontSize: 28 }}>📡</span><div><h2 style={{ fontFamily: "var(--font-hud)", fontSize: 22, color: "var(--green)" }}>SUPPORT H4CKR</h2><p style={{ color: "var(--text-dim)", marginTop: 4, fontSize: 13 }}>Un bug ? Une question ? L'équipe vous répond sous 48h.</p></div></div>
      <div style={{ marginBottom: 28 }}>
        {[{ q: "Ma réponse n'est pas acceptée", a: "Les réponses sont insensibles à la casse. Essayez sans espaces, en MAJUSCULES, en minuscules, et sans accents." }, { q: "Le terminal ne répond pas", a: "Tapez 'help' pour voir les commandes disponibles." }, { q: "L'image ou l'audio ne se charge pas", a: "Les fichiers doivent être générés via generate_assets dans le backend." }, { q: "Comment récupérer mon certificat ?", a: "Le certificat se génère quand vous avez résolu TOUTES les énigmes d'un niveau." }].map((f, i) => (
      ...
      <div className="game-card" style={{ padding: 26 }}>
        ...
        <form onSubmit={submit}>
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 7 }}>{[{ v: "bug", l: "🐛 Bug" }, { v: "suggestion", l: "💡 Suggestion" }, { v: "other", l: "📝 Autre" }].map(o => <button key={o.v} type="button" className={`hud-btn ${form.category === o.v ? "active" : ""}`} onClick={() => setForm(f => ({ ...f, category: o.v }))} style={{ fontSize: 10 }}>{o.l}</button>)}</div>
          </div>
          <div style={{ marginBottom: 14 }}><div style={{ color: "var(--text-dim)", fontFamily: "var(--font-hud)", fontSize: 9, letterSpacing: 2, marginBottom: 6 }}>SUJET</div><input className="game-input" value={form.subject} onChange={e => setForm(f => ({ ...f, subject: e.target.value }))} placeholder="Décrivez brièvement le problème..." required /></div>
          <div style={{ marginBottom: 18 }}><div style={{ color: "var(--text-dim)", fontFamily: "var(--font-hud)", fontSize: 9, letterSpacing: 2, marginBottom: 6 }}>MESSAGE</div><textarea className="game-input" value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))} placeholder="Détaillez votre problème..." rows={5} required style={{ resize: "vertical", minHeight: 110 }} /></div>
...
export default function GamePage() {
  ...
  const isDark = theme === "dark";
  const tabs = [{ id: "game", label: "> JEU" }, { id: "leaderboard", label: "> CLASSEMENT" }, { id: "badges", label: "> BADGES" }, { id: "guide", label: "> GUIDE" }, { id: "contact", label: "> SUPPORT" }] as const;
      ...
      <div style={{ position: "relative", zIndex: 2, minHeight: "100vh" }}>
        ...
        {tab === "game" && (
          <div style={{ display: "flex", padding: "0 28px", background: "var(--hud-bg)", borderBottom: "1px solid var(--border-hud)" }}>
            {[{ label: "PROGRESSION", value: `${pct}%`, sub: `${totalSolved}/${totalEnigmas} énigmes` }, { label: "NIVEAUX", value: levels.length, sub: "chargés" }, { label: "BADGES", value: badgesCount, sub: "obtenus" }, { label: "AGENT", value: user?.pseudo?.slice(0, 12), sub: user?.auth_provider }].map((s, i) => (
